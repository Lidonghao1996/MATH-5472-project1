...
def main():
    ...
    optimizer = optim.Adam(state["model"].parameters(), lr=args.lr, weight_decay=args.weight_decay)
...
def train(state):
    ...
    for epoch in range(state["epoch"]):
        ...
        for batch_idx, inputs in enumerate(dataloader):
            ...
            print("data mean {:.4f} max {:.4f} min {:.4f}".format(inputs.mean(),inputs.max(),inputs.min()) )
